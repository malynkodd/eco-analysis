...
# Must be set before importing pyplot so the Agg backend is active when the
# rendering thread spins up under gunicorn.
matplotlib.use("Agg")
...
from schemas import ReportInput  # noqa: E402
...
def fig_to_image(fig, width_cm=17, height_cm=8):
    buf = io.BytesIO()
    fig.savefig(
        buf, format="png", dpi=150, bbox_inches="tight", facecolor="white", edgecolor="none"
    )
    plt.close(fig)
    buf.seek(0)
...
def chart_npv(data: ReportInput):
    ...
    ax.set_title(
        "Net Present Value (NPV) by Measure",
        fontsize=13,
        fontweight="bold",
        color="#0f4c81",
        pad=12,
...
def chart_co2(data: ReportInput):
    ...
    ax2.set_title(
        "Averted Environmental Damage (K UAH)", fontsize=12, fontweight="bold", color="#6c5ce7"
    )
...
def build_styles():
    ...
    return {
        ...
        "normal": ParagraphStyle(
            "N", parent=styles["Normal"], fontSize=9, spaceAfter=4, fontName=f, textColor=DARK
        ),
        ...
        "center": ParagraphStyle(
            "C", parent=styles["Normal"], fontSize=9, alignment=TA_CENTER, fontName=f
        ),
...
def generate_pdf(data: ReportInput) -> bytes:
    ...
    story.append(Paragraph("Key financial performance indicators for each measure:", s["normal"]))
    ...
    best_idx = next(
        (
            i + 1
            for i, fi in enumerate(data.financial_results)
            if safe(fi.name) == safe(data.best_measure)
        ),
        None,
    )
    if best_idx:
    ...
    if data.ahp_data:
        ...
        story.append(
            make_table(ahp_rank_rows, [2 * cm, 9 * cm, PAGE_W - 11 * cm], header_bg=ACCENT)
        )
    ...
    if data.topsis_data:
        ...
        story.append(
            make_table(
                topsis_rows, [2 * cm, 5 * cm, 4 * cm, 3 * cm, PAGE_W - 14 * cm], header_bg=ACCENT
            )
        )
    ...
    if data.sensitivity_data:
        ...
        story.append(
            Paragraph("Parameters sorted by impact on NPV (highest influence first):", s["normal"])
        )
        ...
        story.append(
            make_table(sens_rows, [1.2 * cm, 5 * cm, 4 * cm, PAGE_W - 10.2 * cm], header_bg=PRIMARY)
        )
        ...
        [
            [
                Paragraph(
                    "RECOMMENDED\nMEASURE",
                    ParagraphStyle(
                        "RB", fontName=fb, fontSize=10, textColor=WHITE, alignment=TA_CENTER
                    ),
                ),
                Paragraph(
                    safe(data.best_measure),
                    ParagraphStyle("RV", fontName=fb, fontSize=16, textColor=PRIMARY),
                ),
            ]
        ],
        colWidths=[3.5 * cm, PAGE_W - 3.5 * cm],
